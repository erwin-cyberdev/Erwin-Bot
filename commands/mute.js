...
export default async function(sock, msg) {
  ...
  const isAuthorized = await isAdmin(sender) || sender.endsWith(process.env.OWNER_NUMBER)
  ...
  try {
    ...
    if (!isBotAdmin) {
      return sock.sendMessage(from, { 
        text: '⚠️ Je dois être administrateur du groupe pour activer le mode muet.'
      })
    }
    ...
    await sock.sendMessage(from, { 
      text: '🔇 *Mode muet activé*\n\nSeuls les administrateurs peuvent maintenant envoyer des messages dans ce groupe.'
    })
    
  } catch (err) {
    console.error('Erreur lors de l\'activation du mode muet:', err)
    
    let errorMessage = '❌ Une erreur est survenue lors de l\'activation du mode muet.'
