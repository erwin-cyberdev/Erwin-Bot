...
- **.gemini <prompt>** — Envoyer un message à Gemini avec un prompt texte.
...
- **.disclaimer** — Afficher l’avertissement légal et les conditions d’utilisation du bot.
...
- **.mistral [mixtral|small] <prompt>** — Envoyer un message à Mistral Small ou Mixtral 8x7B.
...
- **.wiki <recherche>** — Résumer l’article Wikipédia correspondant (requiert accès HTTP à l’API Wikipédia).
- **.roulette** — Jouer à la roulette russe virtuelle.
- **.say <texte>** — Convertir du texte en audio.
- **.left** — Faire quitter le groupe au bot (confirmation envoyée dans le chat avant départ).
- **.ship <nom1> <nom2>** — Estimer l’affinité entre deux membres.
- **.shorten <url>** — Raccourcir une URL.
- **.screenshot <url>** — Capturer une page web et renvoyer la capture d’écran.
- **.song <titre|lien YouTube>** — Télécharger une chanson depuis YouTube.
- **.remind <temps> <message>** — Programmer un rappel (ex: `.remind 10m boire eau`).
...
- **.waifu [type] [catégorie]** — Obtenir une image aléatoire (types: waifu, husbando, etc.; catégories: sfw/nsfw).
